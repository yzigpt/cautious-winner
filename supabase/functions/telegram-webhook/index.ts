...
async function sendGuarantorDealDetails(supabase: any, botToken: string, chatId: number, dealId: string) {
  ...
  if (usernamePattern.test(buyerName)) participantLinks.push({ text: "Открыть покупателя", url: `https://t.me/${buyerName}` });
  if (usernamePattern.test(sellerName)) participantLinks.push({ text: "Открыть продавца", url: `https://t.me/${sellerName}` });
  if (participantLinks.length) actions.push(participantLinks);
